...
export default function Page() {
  ...
  const { data, isLoading } = useProducts({ category: "accessories" });
        ...
        <div className="flex items-end justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              {selectedBrand ? `${selectedBrand.charAt(0).toUpperCase() + selectedBrand.slice(1)} Accessories` : 'All Accessories'}
            </h2>
            <p className="text-sm text-gray-600">
              {selectedBrand ? `Accessories from ${selectedBrand.charAt(0).toUpperCase() + selectedBrand.slice(1)}` : 'Browse the full catalog'}
            </p>
          </div>
        </div>
        {isLoading ? (
