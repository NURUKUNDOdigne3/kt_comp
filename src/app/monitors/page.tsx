...
export default function Page() {
  ...
  const { data, isLoading } = useProducts({ category: "monitors" });
      ...
      {/* <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12" id="shop">
        ...
        {products.featured.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <p className="text-gray-500">No featured monitors available.</p>
          </div>
        )}
      </div> */}

      <div
        className="max-w-7xl mx-auto px-4 mt-5 sm:px-6 lg:px-8"
        id="all-products"
      >
        <div className="flex items-end justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              {selectedBrand ? `${selectedBrand.charAt(0).toUpperCase() + selectedBrand.slice(1)} Monitors` : 'All Monitors'}
            </h2>
            <p className="text-sm text-gray-600">
              {selectedBrand ? `Monitors from ${selectedBrand.charAt(0).toUpperCase() + selectedBrand.slice(1)}` : 'Browse the full catalog'}
            </p>
          </div>
        </div>
        {isLoading ? (
